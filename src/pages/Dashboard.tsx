...
// --- CUSTOM TOOLTIP ESTILO PREMIUM ---
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white/90 dark:bg-zinc-900/90 backdrop-blur-xl p-4 rounded-[1.25rem] shadow-[0_10px_40px_-10px_rgba(0,0,0,0.15)] border border-zinc-200/50 dark:border-white/5 relative z-50">
        <p className="text-[11px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-[0.15em] mb-1.5">{label}</p>
        <p className="text-2xl font-black text-zinc-900 dark:text-white tabular-nums tracking-tighter leading-none">
          {payload[0].value} <span className="text-[13px] font-semibold text-zinc-500 font-sans tracking-normal ml-0.5">un</span>
        </p>
      </div>
    );
  }
...
export default function Dashboard() {
  ...
  const DashboardSkeleton = () => (
    <div className="p-4 md:p-6 lg:p-8 space-y-6 md:space-y-8 bg-zinc-50 dark:bg-zinc-950 min-h-screen pb-24 lg:pb-8 animate-in fade-in duration-1000">
      <div className="flex flex-col gap-2 mb-8">
        <Skeleton className="h-10 w-3/4 max-w-sm rounded-2xl bg-zinc-200/60 dark:bg-white/5" />
        <Skeleton className="h-5 w-1/2 max-w-xs rounded-xl bg-zinc-200/60 dark:bg-white/5" />
      </div>
      <div className="grid gap-4 md:gap-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-4">
        <Skeleton className="md:col-span-2 h-[240px] rounded-[2.5rem] bg-zinc-200/60 dark:bg-white/5" />
        <Skeleton className="h-[240px] rounded-[2.5rem] bg-zinc-200/60 dark:bg-white/5" />
        <Skeleton className="h-[240px] rounded-[2.5rem] bg-zinc-200/60 dark:bg-white/5" />
      </div>
      <div className="grid gap-4 md:gap-6 grid-cols-1 xl:grid-cols-3">
        <Skeleton className="xl:col-span-2 h-[380px] rounded-[2.5rem] bg-zinc-200/60 dark:bg-white/5" />
        <Skeleton className="h-[380px] rounded-[2.5rem] bg-zinc-200/60 dark:bg-white/5" />
      </div>
    </div>
  );
  ...
  const Header = ({ title, subtitle }: { title: string; subtitle: string }) => (
    <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-6 md:mb-8 animate-in fade-in slide-in-from-top-6 duration-1000 ease-out">
      <div>
        <div className="flex items-center gap-2 text-red-600 dark:text-red-500 mb-2 opacity-90">
          <timeState.Icon className="h-5 w-5" strokeWidth={2.5} />
          <span className="text-sm font-bold tracking-tight">{timeState.greeting}</span>
        </div>
        <h1 className="text-3xl md:text-[44px] font-black text-zinc-900 dark:text-white tracking-tighter leading-none">
          {title}
        </h1>
        <p className="text-zinc-500 dark:text-zinc-400 text-sm md:text-base mt-3 font-medium tracking-tight">
          {subtitle}
        </p>
      </div>
      <div className="hidden md:flex items-center gap-2 bg-white/60 dark:bg-white/5 backdrop-blur-xl px-5 py-2.5 rounded-2xl text-[13px] font-bold text-zinc-600 dark:text-zinc-300 border border-zinc-200/50 dark:border-white/5 shadow-sm">
        <Calendar className="h-4 w-4 text-red-500" strokeWidth={2.5} />
  ...
  const PremiumValueCard = ({ className = "" }: { className?: string }) => (
    <Card className={`rounded-[2.5rem] border-none shadow-[0_24px_48px_-12px_rgba(220,38,38,0.3)] dark:shadow-[0_24px_48px_-12px_rgba(0,0,0,1)] bg-gradient-to-br from-zinc-950 via-red-950/40 to-zinc-900 relative overflow-hidden flex flex-col justify-between group hover:-translate-y-1.5 transition-all duration-500 ease-out cursor-pointer active:scale-[0.98] ${className}`}>
      ...
      <div className="absolute top-0 right-0 w-[300px] h-[300px] rounded-full bg-red-500/20 blur-[80px] pointer-events-none mix-blend-screen group-hover:bg-red-500/30 transition-colors duration-1000" />
      ...
      <CardContent className="p-7 md:p-8 flex flex-col h-full justify-between relative z-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5 bg-white/5 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/10 shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)]">
            <span className="relative flex h-2 w-2">
              <span className={`animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75 ${isRefetching ? 'duration-75' : 'duration-1000'}`}></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-red-500"></span>
            </span>
            <Wallet className="h-4 w-4 text-red-50 opacity-90" strokeWidth={2.5} />
            <h2 className="text-[11px] font-bold tracking-[0.2em] uppercase text-red-100/90 pt-0.5">
              Patrimônio
            </h2>
          </div>
        ...
        <div className="mt-8 flex flex-col">
          ...
          <p className="text-[13px] font-semibold text-red-200/60 mt-3 tracking-wide uppercase">
            Capital Imobilizado
          </p>
        </div>
      </CardContent>
  ...
  const MetricCard = ({ title, value, icon: Icon, colorClass, desc, onClick, className = "" }: any) => (
    <Card onClick={onClick} className={`bg-zinc-50/80 dark:bg-zinc-900/80 backdrop-blur-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04),inset_0_1px_1px_rgba(255,255,255,1)] dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] border border-zinc-200/60 dark:border-white/5 rounded-[2.5rem] cursor-pointer hover:-translate-y-1.5 hover:shadow-[0_16px_40px_rgb(0,0,0,0.08)] transition-all duration-500 ease-out group active:scale-[0.96] overflow-hidden ${className}`}>
      <CardContent className="p-7 md:p-8 flex flex-col justify-between h-full relative z-10">
        <div className="flex items-center justify-between">
          ...
          <div className="h-10 w-10 rounded-2xl bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center group-hover:bg-zinc-200 dark:group-hover:bg-zinc-700 transition-colors duration-500">
        ...
        <div className="mt-8">
          <div className="text-[44px] md:text-[52px] font-black text-zinc-900 dark:text-white tracking-tighter leading-none">
  ...
  const renderBentoDashboard = (isAdmin = false) => {
    // Cores alinhadas com a paleta Tailwind vermelha
    const volumeData = [
      { name: "Catálogo", value: stats?.totalProducts || 0, color: "#DC2626" }, // red-600
      { name: "Pedidos", value: stats?.totalRequests || 0, color: "#EF4444" }, // red-500
      { name: "Saídas", value: stats?.totalSeparations || 0, color: "#F87171" }, // red-400
    ];
    ...
    const stockHealthData = [
      { name: "Regular", value: healthyStock, color: "#DC2626" }, // red-600
      { name: "Crítico", value: lowStock > 0 ? lowStock : 0.01, color: "#27272A" }, // zinc-800
    ];

    return (
      <div className="p-4 md:p-6 lg:p-8 bg-zinc-50 dark:bg-zinc-950 min-h-screen pb-24 lg:pb-8 selection:bg-red-500/30">
        <Header 
        ...
        <div className="grid gap-4 md:gap-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-4 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-150 fill-mode-both ease-out mb-4 md:mb-6">
          ...
          <MetricCard 
            ...
            colorClass={{ bg: 'bg-amber-50 dark:bg-amber-500/10', border: 'border-amber-100 dark:border-amber-500/20', text: 'text-amber-500' }}
          />
          
          <MetricCard 
            title="A Rever" value={stats?.openRequests || 0} icon={Activity} 
            desc="Pedidos pendentes" onClick={() => navigate('/requests')}
            colorClass={{ bg: 'bg-red-50 dark:bg-red-500/10', border: 'border-red-100 dark:border-red-500/20', text: 'text-red-500' }}
          />
        </div>
        ...
        <div className="grid gap-4 md:gap-6 grid-cols-1 xl:grid-cols-3 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300 fill-mode-both ease-out">
          ...
          {/* BAR CHART */}
          <Card className="xl:col-span-2 bg-zinc-50/80 dark:bg-zinc-900/80 backdrop-blur-2xl shadow-[0_8px_30px_rgb(0,0,0,0.03),inset_0_1px_1px_rgba(255,255,255,1)] dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] border border-zinc-200/60 dark:border-white/5 rounded-[2.5rem]">
            <CardContent className="p-6 md:p-8">
              <div className="flex items-center gap-2 mb-8">
                <BarChart3 className="h-5 w-5 text-zinc-400" />
                <h3 className="text-[12px] font-bold text-zinc-400 uppercase tracking-[0.2em] pt-0.5">
                  Volume Operacional
                </h3>
              </div>
          ...
          {/* PIE CHART (Donut) */}
          <Card className="bg-zinc-50/80 dark:bg-zinc-900/80 backdrop-blur-2xl shadow-[0_8px_30px_rgb(0,0,0,0.03),inset_0_1px_1px_rgba(255,255,255,1)] dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] border border-zinc-200/60 dark:border-white/5 rounded-[2.5rem]">
            <CardContent className="p-6 md:p-8 flex flex-col h-full">
              ...
              <div className="flex-1 min-h-[280px] relative flex items-center justify-center">
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-[40px] font-black text-zinc-900 dark:text-white tracking-tighter leading-none">
                    {Math.round((healthyStock / totalStock) * 100)}<span className="text-2xl">%</span>
                  </span>
                  <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-[0.2em] mt-2">Saudável</span>
                </div>
    ...
    <div className="p-4 md:p-6 lg:p-8 bg-zinc-50 dark:bg-zinc-950 min-h-screen pb-24 lg:pb-8 selection:bg-red-500/30">
      ...
      <div className="grid gap-4 md:gap-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-4 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-150 fill-mode-both ease-out">
        ...
        <MetricCard 
          ...
          colorClass={{ bg: 'bg-zinc-100 dark:bg-zinc-800/50', border: 'border-zinc-200 dark:border-zinc-700', text: 'text-zinc-600 dark:text-zinc-400' }}
        />
        <MetricCard 
    ...
    <div className="p-4 md:p-6 lg:p-8 bg-zinc-50 dark:bg-zinc-950 min-h-screen pb-24 lg:pb-8 selection:bg-red-500/30">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-150">
        ...
        <Card 
          ...
          className="bg-red-600 dark:bg-red-700 text-white cursor-pointer hover:-translate-y-1.5 hover:shadow-[0_16px_40px_rgba(220,38,38,0.4)] transition-all duration-300 active:scale-95 border-none rounded-[2.5rem] overflow-hidden relative group"
        >
        ...
        <Card 
          ...
          className="bg-zinc-50/80 dark:bg-zinc-900/80 backdrop-blur-2xl cursor-pointer hover:-translate-y-1.5 hover:shadow-[0_16px_40px_rgb(0,0,0,0.08)] transition-all duration-300 active:scale-95 border border-zinc-200/60 dark:border-white/5 rounded-[2.5rem] shadow-sm group"
        >
          <CardContent className="p-8 md:p-10 flex flex-col items-center justify-center text-center h-full">
            <div className="h-16 w-16 bg-zinc-100 dark:bg-white/5 text-zinc-500 dark:text-zinc-400 rounded-full flex items-center justify-center mb-6 group-hover:bg-zinc-200 dark:group-hover:bg-white/10 transition-colors">
              <ClipboardList className="h-8 w-8" strokeWidth={2} />
            </div>
            <h2 className="text-2xl font-black tracking-tight text-zinc-900 dark:text-white mb-2">Meus Pedidos</h2>
  ...
  const renderDashboard = () => {
    ...
    switch (profile?.role) {
      ...
      default: return <div className="p-8 text-center text-zinc-500 font-medium">A carregar perfil...</div>;
    }
  };
